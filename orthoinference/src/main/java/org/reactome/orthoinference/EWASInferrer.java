...
import org.reactome.core.model.ReferenceRNASequence;
...
public class EWASInferrer {
	...
	private static Map<String, GKInstance> referenceDNASequenceIdenticals = new HashMap<String,GKInstance>();
	private static Map<String, GKInstance> referenceRNASequenceIdenticals = new HashMap<String,GKInstance>();
	...
	public static List<GKInstance> inferEWAS(GKInstance ewasInst) throws InvalidAttributeException, Exception
	{
		...
		*: already exists
		.get EWAS
		.get homologues - ties in with EventsInferrer.readHomologueMappingFile()
			.look for appropriate type of homolog(s)
				[w].allows for id-switching template (G’s for T’s - very specific to PR, and a hack) - this is ok in orthopair file
				[w].also refDB switching, b/c Ensemble-Gramene: won't be in orthopair files - only do Ensemble-Gramene for now
		x.what kind is EWAS this? (class: RGP/RNA/DNA)
			-.curated DNA (new case)
				.check for pre-existence (avoid dupes)
				.make projected RefDNASeq
				.make homolog EWAS for DNA
			-.curated RNA (new case)
				.check for pre-existence (avoid dupes)
				*.make projected RefDNASeq
				.make homolog EWAS for RNA
			-.curated RGP (current case)
				.make RefRNASeq, too, just for grins? - wait on this
				*.make projected RefDNASeq
		...
		*/
		...
		if (((GKInstance) ewasInst.getAttributeValue(referenceEntity)).getAttributeValue(identifier) != null) {  // protect from NPE for incomplete EWAS
			GKInstance curReferenceEntity = (GKInstance) ewasInst.getAttributeValue(referenceEntity);
			logger.info("Current RefEntity " + curReferenceEntity + " for EWAS " + ewasInst);
			String referenceEntityId = ((GKInstance) ewasInst.getAttributeValue(referenceEntity)).getAttributeValue(identifier).toString().toUpperCase();
			if (homologueMappings.get(referenceEntityId) != null) // if we can find a homolog in the loaded orthopair files
			{
				//if (referenceEntityId.equals("OS06G0712700"))
				//if (referenceEntityId.equals("OS02T0244100"))
				//	logger.info("found ya");
				...
				for (String homologue : homologueMappings.get(referenceEntityId)) {
					...
					// DNA Section
					if (curReferenceEntity.getSchemClass().isa(ReferenceDNASequence)) {
						logger.info("DNA hit: " + curReferenceEntity);
						//if (checkValidSpeciesProtein(homologueId)) { // TODO: determine if necessary; depends on whether DNA/RNA ids are added to {prj_species}_gene_protein_mapping.txt
						GKInstance infRefDNAInst = null;
						if (referenceDNASequenceIdenticals.get(homologueId) == null) { // check for pre-existence (avoid dupes)
							logger.info("Found a DNA homolog: " + homologueId);

							// make projected RefDNASeq; Creates ReferenceDNASequence instance from ReferenceEntity
							List<GKInstance> inferredReferenceDNAInstances = createReferenceDNASequence(homologueId);
							infRefDNAInst = inferredReferenceDNAInstances.get(0); // only one allowed in the model; can only use one in this context, anyway

							referenceDNASequenceIdenticals.put(homologueId, infRefDNAInst);
						} else {
							infRefDNAInst = referenceDNASequenceIdenticals.get(homologueId);
						}
						// make homolog EWAS for DNA
						GKInstance infEWASInst = InstanceUtilities.createNewInferredGKInstance(ewasInst);
						infEWASInst.addAttributeValue(referenceEntity, infRefDNAInst);
						...
						// added gene to the display name to diff from proteins and transcripts, in keeping with curator practice
						String ewasDisplayName = (String) infEWASInst.getAttributeValue(name) + " gene" + (curCompartment.length() > 0 ? " [" + curCompartment + "]" : "");
					...
					// RNA Section
					if (curReferenceEntity.getSchemClass().isa(ReferenceRNASequence)) {
						logger.info("RNA hit: " + curReferenceEntity);
						GKInstance infRefRNAInst = null;
						if (referenceRNASequenceIdenticals.get(homologueId) == null) {  // check for pre-existence (avoid dupes)
							logger.info("Found an RNA homolog: " + homologueId);

							// make projected RefRNASeq; Creates ReferenceRNASequence instance from ReferenceEntity
							List<GKInstance> inferredReferenceRNAInstances = createReferenceRNASequence(homologueId);
							infRefRNAInst = inferredReferenceRNAInstances.get(0); // only one allowed in the model; can only use one in this context, anyway

							referenceRNASequenceIdenticals.put(homologueId, infRefRNAInst);
						} else {
							infRefRNAInst = referenceRNASequenceIdenticals.get(homologueId);
						}
						// make homolog EWAS for RNA
						GKInstance infEWASInst = InstanceUtilities.createNewInferredGKInstance(ewasInst);
						infEWASInst.addAttributeValue(referenceEntity, infRefRNAInst);
						infEWASInst.addAttributeValue(name, homologueId);
						String curCompartment = "";
						if (ewasInst.getAttributeValue(compartment) != null) {
							curCompartment = ((GKInstance) ewasInst.getAttributeValue(compartment)).getDisplayName();
						}
						// added transcript to the display name to diff from proteins and genes, in keeping with curator practice
						String ewasDisplayName = (String) infEWASInst.getAttributeValue(name) + " transcript" + (curCompartment.length() > 0 ? " [" + curCompartment + "]" : "");
						infEWASInst.setAttributeValue(_displayName, ewasDisplayName);
					...
					// RGP Section
						...
						if (curReferenceEntity.getSchemClass().isa(ReferenceGeneProduct)) {
							logger.info("RGP hit: " + curReferenceEntity);
	...
	// Creates ReferenceRNASequence instance based on ENSG identifier mapped to protein. Creates an instance for the primary database and an alternate, if it exists.
	private static List<GKInstance> createReferenceRNASequence(String homologueId) throws InvalidAttributeException, InvalidAttributeValueException, Exception
	{
		List<GKInstance> referenceRNAInstances = new ArrayList<GKInstance>();
		List<String> ensgIds = ensgMappings.get(homologueId);
		for (String ensgId : ensgIds)
		{
			SchemaClass referenceRNAClass = dba.getSchema().getClassByName(ReferenceRNASequence);
			GKInstance referenceRNAInst = new GKInstance(referenceRNAClass);
			referenceRNAInst.setDbAdaptor(dba);
			referenceRNAInst.addAttributeValue(created, instanceEditInst);
			referenceRNAInst.addAttributeValue(identifier, ensgId);
			referenceRNAInst.addAttributeValue(referenceDatabase, ensgDbInst);
			referenceRNAInst.addAttributeValue(species, speciesInst);
			referenceRNAInst.setAttributeValue(_displayName, ensgDbInst.getAttributeValue(name) + ":" + ensgId);

			// also add RefDNASeq for this RNA, if it doesn't already exist (but no EWAS; this is just for completeness)
			GKInstance infRefDNAInst = null;
			if (referenceDNASequenceIdenticals.get(homologueId) == null) { // check for pre-existence (avoid dupes)
				logger.info("Need a RefDNASeq for this RefRNASeq: " + homologueId);
			...
			if (infRefDNAInst != null) {
				referenceRNAInst.addAttributeValue(referenceGene, infRefDNAInst);
			}

			referenceRNAInst = InstanceUtilities.checkForIdenticalInstances(referenceRNAInst,null);
			referenceRNAInstances.add(referenceRNAInst);
			if (altRefDbExists)
			{
				GKInstance alternateRefRNAInst = new GKInstance(referenceRNAClass);
				alternateRefRNAInst.setDbAdaptor(dba);
				String altDbIdentifier = (String) ensgId;
				if (altRefDbId != null)
				{
					altDbIdentifier = altDbIdentifier.replaceAll(altRefDbId, "");
				}
				alternateRefRNAInst.addAttributeValue(created, instanceEditInst);
				alternateRefRNAInst.addAttributeValue(identifier, altDbIdentifier);
				alternateRefRNAInst.addAttributeValue(referenceDatabase, alternateDbInst);
				alternateRefRNAInst.addAttributeValue(species, speciesInst);
				alternateRefRNAInst.setAttributeValue(_displayName, alternateDbInst.getAttributeValue(name) + ":" + ensgId);
				alternateRefRNAInst = InstanceUtilities.checkForIdenticalInstances(alternateRefRNAInst, null);
				referenceRNAInstances.add(alternateRefRNAInst);
			}
		}
		return referenceRNAInstances;
	}
...
*/
	...
	public static void resetVariables()
	{
		...
		referenceDNASequenceIdenticals = new HashMap<>();
		referenceRNASequenceIdenticals = new HashMap<>();
