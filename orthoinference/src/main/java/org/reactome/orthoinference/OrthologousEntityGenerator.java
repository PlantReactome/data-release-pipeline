...
*/
	...
	// are also subject to the 'countDistinctProteins' function. The result from this needs to have at least <inferenceThreshold>% of total proteins to be inferrable for inference to continue.
...
//			int complexMax = complexProteinCounts.get(2); // Doesn't get used, since MaxHomologue isn't a valid attribute anymore.
			...
			// Filtering based on results of ProteinCounts and threshold
			int percent = 0;
			...
			if (!override)
			{
				if ((complexTotalProteinCounts > 0 && complexInferrableProteinCounts == 0) || percent < EventsInferrer.inferenceThreshold)
				{
